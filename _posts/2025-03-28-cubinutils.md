...
## Architecture-specific 和 Family-specific feature

从 Hopper 架构开始，出现 architecture-specific 特性（a结尾），仅能在特定硬件架构上使用。使用后，ptx 或者 cubin 不再向前兼容。

```bash
$ nvcc -gencode arch=compute_90a,code=sm_90a -c kernel.cu
```

从 Blackwell 开始，引入 family-specific 特性（f结尾），能在 major compute capability version 一样，minor compute capability 一样或更高的硬件架构上使用。
...
[^4]: [NVIDIA Blackwell and NVIDIA CUDA 12.9 Introduce Family-Specific Architecture Features](https://developer.nvidia.com/blog/nvidia-blackwell-and-nvidia-cuda-12-9-introduce-family-specific-architecture-features/?linkId=100000363800385#putting_it_into_practice)
